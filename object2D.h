...
public:
	...
	void SetPresentRevolutionAngle(const float fAngle);
	const D3DXVECTOR3 GetRevolutionCenter(void);									//公転の中心点の取得処理
	void AddRevolutionSpeed(const float fSpeed);
	...
	void MoveTexCoordinates(const D3DXVECTOR2 move);
	bool GetFlipX(void);
	void FlipX(void);
	void FlipY(void);
...
private:
	...
	D3DXVECTOR2 m_textureTranslation;
	bool m_bFlipX;
	bool m_bFlipY;
